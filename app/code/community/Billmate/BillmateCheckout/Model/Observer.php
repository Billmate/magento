...
class Billmate_BillmateCheckout_Model_Observer extends Mage_Core_Model_Abstract
{
    /**
	...
	{	/** @var $order Mage_Sales_Model_Order */
        ...
        if(!in_array($method,array('billmatecheckout','billmatepartpayment'))) return;

		$session = Mage::getSingleton("core/session",  array("name"=>"frontend"));
		$liveid = $session->getData("billmatecheckout_id");
		$session->unsetData('billmatecheckout_id');
    ...
    public function salesOrderPaymentPlaceEnd(Varien_Event_Observer $observer)
    {
        ...
        if ($payment->getMethodInstance()->getCode() != 'billmatecheckout') {
            return;
        }
        ...
        $info->setAdditionalInformation('billmatecheckout_fee',$quote->getFeeAmount());
		$info->setAdditionalInformation('billmatecheckout_fee_tax',$quote->getFeeTaxAmount());
        $info->save();
    }
